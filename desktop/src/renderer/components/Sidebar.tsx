      ...
      <Show when={!collapsed()}>
        <div class="px-5 pt-5 pb-3">
          ...
          <div class="flex items-center justify-between">
            <h1 class="text-xl font-bold text-[var(--text-primary)] tracking-tight">Desktop Agent</h1>
            <button data-component="icon-button" data-size="sm" onClick={() => setCollapsed(true)} title="折叠">
        ...
        <div class="px-4 pb-3">
          <button
            ...
            onMouseEnter={(event) => {
              event.currentTarget.style.background = "var(--brand-soft)";
              event.currentTarget.style.borderColor = "color-mix(in srgb,var(--brand) 28%,var(--border-light))";
              event.currentTarget.style.transform = "translateY(-1px)";
              event.currentTarget.style.boxShadow = "0 8px 18px rgba(67,51,34,.08)";
            }}
            onMouseLeave={(event) => {
        ...
        <div class="flex-1 overflow-y-auto px-3 pb-2">
          <p class="text-xs font-medium text-[var(--text-tertiary)] uppercase tracking-wider px-2 py-1.5">历史任务</p>
              ...
              <For each={filtered()}>
                {(item) => (
                  <div
                    ...
                    style={{ background: props.selectedId === item.id ? "var(--brand-soft)" : "transparent" }}
                  ...
                  >
                    <div class="flex items-center gap-2">
                      ...
                      <button
                        ...
                        onMouseEnter={(event) => { event.currentTarget.style.background = "var(--danger-soft)"; event.currentTarget.style.color = "var(--danger)"; }}
                        onMouseLeave={(event) => { event.currentTarget.style.background = "transparent"; event.currentTarget.style.color = "var(--text-tertiary)"; }}
                        onClick={(event) => { event.stopPropagation(); props.onDeleteHistory(item.id); }}
                        title="删除"
                      >×</button>
                    </div>
                  </div>
        ...
        <div class="border-t px-4 py-3" style="border-color:var(--border-light)">
          <div class="flex items-center gap-2">
            <div
              class="flex items-center gap-3 rounded-lg p-2.5 cursor-default transition-colors hover:bg-[var(--bg-hover)] min-w-0"
              style="flex:1"
              onClick={props.onOpenSettings}
            >
              <div class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold text-white" style="background:var(--brand)">
                {(props.currentUser?.nickname || props.currentUser?.email || "A").slice(0, 1).toUpperCase()}
              </div>
              <div class="flex-1 min-w-0">
                <p class="text-sm font-medium text-[var(--text-primary)] truncate">账号与设置</p>
                <p class="text-xs text-[var(--text-tertiary)] truncate">{props.currentUser?.email || "未登录 · API 配置"}</p>
              </div>
            </div>
            <button
              data-component="button"
              ...
              data-size="sm"
              style={{
                "border-color": props.currentUser?.plan === "pro" ? "rgba(201,147,58,.38)" : "rgba(37,99,235,.22)",
                background: props.currentUser?.plan === "pro"
                  ? "linear-gradient(135deg, rgba(250,204,21,.2), rgba(37,99,235,.12))"
                  : "linear-gradient(135deg, rgba(37,99,235,.12), rgba(201,147,58,.14))",
                color: props.currentUser?.plan === "pro" ? "#805600" : "var(--brand)",
                "flex-shrink": 0,
                "font-weight": 800,
                "letter-spacing": "0",
              ...
              onMouseEnter={(event) => {
                event.currentTarget.style.transform = "translateY(-1px)";
                event.currentTarget.style.boxShadow = "0 10px 22px rgba(37,99,235,.14)";
                event.currentTarget.style.borderColor = props.currentUser?.plan === "pro" ? "rgba(201,147,58,.55)" : "rgba(37,99,235,.34)";
              }}
              onMouseLeave={(event) => {
                event.currentTarget.style.transform = "translateY(0)";
                event.currentTarget.style.boxShadow = "none";
                event.currentTarget.style.borderColor = props.currentUser?.plan === "pro" ? "rgba(201,147,58,.38)" : "rgba(37,99,235,.22)";
              }}
              onClick={(event) => {
                event.stopPropagation();
                props.onOpenPurchase();
              }}
              title={props.currentUser?.plan === "pro" ? "Pro 已开通" : "升级 Pro"}
            >
              {props.currentUser?.plan === "pro" ? "Pro" : "升级"}
            </button>
          </div>
        </div>
      </Show>
